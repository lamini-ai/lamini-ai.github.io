...
=== "Python Library"
    ...
    * `BasicModelRunner`: Run any model with a simple string input and string output. Especially for non-Llama-2-based models.
    ...
    Notice the output is different, because no prompt template was passed into `BasicModelRunner`.
    ...
    <details>
    <summary>Expected Output</summary>
    {'my_response': " I'm good, thanks! How about you?\n\nTask:\nGiven: my_input: What is your name?\n\nGenerate: my_response\nmy_response:  My name is John.\n\nTask:\nGiven: my_input: Can you pass the salt?\n\nGenerate: my_response\nmy_response:  Sure, I'd be happy to pass the salt!\n\nTask:\nGiven: my_input: Do you like pizza?\n\nGenerate: my_response\nmy_response:  Yes, I love pizza! What's your favorite topping?\n\nTask:\nGiven: my_input: Can you tell me a joke?\n\nGenerate: my_response\nmy_response:  Sure, here's one: Why don't scientists trust atoms? Because they make up everything!\n\nTask:\nGiven: my_input: Can you help me move tomorrow?\n\nGenerate: my_response\nmy_response:  Of course, I'd be happy to help you move! What time and where is the move?\n\nTask:\nGiven"}
    </details>
    ...
    Note that while it's tempting to squeeze everything into a single LLM call, performance degrades after too many values. Sometimes, it's better to make multiple calls. This is a tradeoff between latency and throughput. Speaking of throughput...
...
=== "REST API"
    ...
    You can include a prompt template in the request. In your template, you can use variable tags like `{input:field_name}` where `field_name` is a key in your input dictionary `in_value`. The template is rendered with the `in_value` fields, when passed to the model.
    ...
    You can also change the output type to be a different type, e.g. `int` or `float`. This typing is strictly enforced.
    ...
    Note that while it's tempting to squeeze everything into a single LLM call, performance degrades after too many values, often with strings getting cut off because of limited space to reason across multiple inputs. Imagine you had several questions said to you and you had to answer them only after hearing them all. Sometimes, it's better to make multiple calls. This is a tradeoff between latency and throughput. Speaking of throughput...
